// melakukan perhitungan enkripsi dengan mengalikan plaintext dengan matriks kunci
...
// melakukan perhitungan dekripsi dengan mengalikan ciphertext dengan matriks kunci yang sudah melalui proses invers
...
// mendapatkan pesan dan kunci yang dimasukkan oleh pengguna dan mengacaknya
...
// melakukan perhitungan dekripsi dengan mengalikan ciphertext dengan matriks kunci yang sudah melalui proses invers
function decrypt2(ciphertext, k) {
    keys = k.split(" ");
    if (ciphertext.length % 2 == 1) { alert("ciphertext is not divisible by 2 (wrong algorithm?)"); return; }
    if (keys.length != 4) { alert("key should consist of 4 integers"); return; }
    for (i = 0; i < 4; i++) keys[i] = keys[i] % 26;
...
// mendapatkan pesan dan kunci yang dimasukkan oleh pengguna dan menguraikannya
function decipherButtonFunction() {
    ...
    var result = decrypt2(message, enteredKey);
    document.getElementById("result").value = result;
}
